...
int main( int argc, char *argv[] )
{
    ...
    std::cout << std::endl << "ReAddress command line is:" << std::endl;
...
void GetDefault(){
    ...
    while ( !finish )
    {
        ...
        for ( auto it = addressVar.begin(); it != addressVar.end(); it++ )
        {
            uint64_t address = it->first;
            // std::cout << "Address: 0x" << std::hex << address << std::dec << std::endl;
            //将address转换为行、列、rank、bank、channel、subarray
            uint64_t channel, rank, bank, row, col, subarray;
            translator->Translate( address, &row, &col, &rank, &bank, &channel, &subarray );
            // std::cout << "Translated Address: "
            //     << "Bank: " << bank << ", Rank: " << rank << std::endl;
            //
            int bankId = rank * banks + bank; //bankId = rank * banks + bank
            //判断banklists[bankId]中是否已经有address，若没有则插入
            bool isExist = false;
            ...
            if ( isExist )
                continue; //如果已经存在，则跳过
            //如果不存在，则将address插入到banklists[bankId]中
            if( bankId < banknum )
            {
                banklists[bankId].push_back(address);
                // std::cout << "Address 0x" << std::hex << address << std::dec 
                    // << " placed in Bank " << bankId << std::endl;
            }
            else
            {
                std::cout << "Bank ID out of range: " << bankId << std::endl;
            }
        }
    }
    ...
    for( int i = 0; i < banknum; i++ )
    {
        objnum += banklists[i].size();
    }
    std::cout << "Total number of objs: " << objnum << std::endl;
    for( int i = 0; i < banknum; i++ )
    {   //输出百分比
        std::cout << "Bank " << i << " has " << banklists[i].size() << " objs." << " "
            << "Percentage: " << (double)banklists[i].size() / objnum * 100 << "%" << std::endl;
    }
}

void DataPlacement(){ //trace文件最后一行必须有一行空行
    bool finish = false;
    while ( !finish )
    {
        ...
        for( int i=0 ; i < count-1 ; i++ ){ //遍历访问序列，构建访问图
        ...
        for( int i=0 ; i < varnum ; i++ ){  //对每个顶点求顶点权重
        ...
        for ( auto it = varWeights.begin(); it != varWeights.end(); it++ )  //依次对该组地址确定放置位置
        {
            ...
            for (unsigned int i = 1; i < bankWeight.size(); i++) //令minbank指向与当前address关联度最小的bank的序号（若有多个关联度最小的bank，选其中已存放数据量最少的bank）
            {
                ...
                // if(bankWeight[minbank] > bankWeight[i] || (bankWeight[minbank] == bankWeight[i] && banklists[minbank].size() > banklists[i].size())){   //负载均衡
                //     minbank = i;
                // }
            }
...
void RewriteAddress()
{
    ...
    std::string insertStr = "_ra";  //要插入的字符串
    ...
    {
        std::string line;
        while (std::getline(inFile, line)) { // 逐行读取文件内容，存到line中
